...
test("tracked generated or cache artifacts are rejected while owned fixtures remain allowed", () => {
  ...
  assert.deepEqual(issues.map((issue) => issue.path), [".next/cache/build.json", "artifacts/e2e/result.zip", "dist/server.js"]);
  for (const issue of issues) {
