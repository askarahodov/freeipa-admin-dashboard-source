...
export type EncryptedBackupExportRequest = {
  domains: PortalBackupDomain[];
  password: string;
};

export type EncryptedBackupDocument = {
  manifest: PortalBackupManifest;
  payloads: Record<string, EncryptedPayloadEnvelope>;
  summary: { entries: number; records: number; bytes: number };
};

export type EncryptedBackupExportOptions = EncryptedBackupExportRequest & {
  schemaVersion: number;
  createdAt?: string;
  iterations?: number;
  salt?: string;
  random?: BackupCryptoRandom;
  ivForDomain?: (domain: PortalBackupDomain) => string;
};
...
function requestInvalid(message: string): never {
  throw new BackupEncryptedExportError("backup_request_invalid", 400, message);
}
...
export function parseEncryptedBackupExportRequest(value: unknown): EncryptedBackupExportRequest {
  ...
  if (Object.keys(value).some((key) => key !== "domains" && key !== "password") || !Object.hasOwn(value, "domains") || !Object.hasOwn(value, "password")) {
    requestInvalid("Encrypted backup export request contains invalid fields");
  }
  if (!Array.isArray(value.domains) || value.domains.length === 0 || value.domains.some((domain) => typeof domain !== "string")) {
    requestInvalid("Backup domains must be a non-empty array");
  }
  ...
  for (const domain of requested) {
    if (!PORTAL_BACKUP_DOMAINS.includes(domain as PortalBackupDomain)) requestInvalid("Unsupported backup domain");
  }
  try {
    validateBackupPassword(value.password);
  } catch {
    requestInvalid("Backup password is invalid");
  }
  return {
...
function normalizedFailure(error: unknown): BackupEncryptedExportError {
  ...
  if (error instanceof BackupEncryptionError) {
    return new BackupEncryptedExportError(error.code, error.status, error.message);
  }
...
export async function exportEncryptedBackup(
  ...
  try {
    ...
    for (const domain of parsed.domains) {
      ...
      if (!exporter || exporter.domain !== domain || exporter.path !== path) {
        throw new BackupEncryptedExportError("backup_schema_incompatible", 409, "Backup domain is unavailable");
      }
      ...
      if (records !== exported.records) {
        throw new BackupEncryptedExportError("backup_schema_incompatible", 409, "Backup record count is inconsistent");
      }
