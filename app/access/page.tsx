  ...
  return (
    <main className="access-shell">
      ...
      <form className="access-panel access-create-form" onSubmit={createUser}>
        ...
        <div className="ds-field"><label htmlFor="access-create-username">Логин</label><input id="access-create-username" className="ds-input" value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} placeholder="operator01" required /></div>
        <div className="ds-field"><label htmlFor="access-create-display-name">Отображаемое имя</label><input id="access-create-display-name" className="ds-input" value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} placeholder="Оператор портала" /></div>
        <div className="ds-field"><label htmlFor="access-create-password">Пароль</label><input id="access-create-password" className="ds-input" type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} minLength={12} autoComplete="new-password" required /><span className="ds-field-helper">От 12 до 256 символов</span></div>
        <div className="ds-field"><label htmlFor="access-create-confirmation">Подтверждение</label><input id="access-create-confirmation" className="ds-input" type="password" value={form.confirmation} onChange={(event) => setForm({ ...form, confirmation: event.target.value })} minLength={12} autoComplete="new-password" required /></div>
        <div className="ds-field"><label htmlFor="access-create-role">Роль</label><select id="access-create-role" className="ds-select" value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value as PortalRole })}><option value="viewer">Наблюдатель</option><option value="operator">Оператор</option><option value="admin">Администратор</option></select></div>
