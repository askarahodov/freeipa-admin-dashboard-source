...
import { PRODUCT_NAV_GROUPS, isProductNavItemActive, type ProductNavItem, type ProductNavItemId } from "./navigation";
...
export function AppShell({ currentPath, visibleItemIds, badges = {}, onNavigate, brand, systemStatus, header, children }: AppShellProps) {
              ...
              <section className={styles.group} key={group.id} aria-label={group.label || undefined}>
                ...
                <div className={styles.groupItems}>
                  {items.map((item) => {
                    ...
                    return (
                      <button className={`${styles.navItem} ${active ? styles.active : ""}`} type="button" aria-current={active ? "page" : undefined} onClick={() => onNavigate(item)} key={item.id}>
                        <ProductIcon name={item.icon} className={styles.navIcon} />
                        <span className={styles.navLabel}>{item.label}</span>
                        {badge !== undefined && badge !== null && badge !== "" ? <span className={styles.badge} aria-label={`${item.label}: ${badge}`}>{badge}</span> : null}
                      </button>
                    );
                  })}
                </div>
