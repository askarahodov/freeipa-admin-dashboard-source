...
Не создавайте второй документ, повторяющий тот же contract. Перед созданием нового файла проверьте [`SOURCE_OF_TRUTH.md`](../reference/SOURCE_OF_TRUTH.md) и существующую документацию.
...
Порядок разрешения противоречий определён в `docs/README.md` и реестре [`SOURCE_OF_TRUTH.md`](../reference/SOURCE_OF_TRUTH.md). Documentation policy не может переопределить фактический security/runtime contract кодом; её задача — не допускать расхождения документации с ним.
