...
function definition(name: string, columns: readonly string[], primaryKey: readonly string[], sourceTable = name): FullBackupTableDefinition {
  return { name, sourceTable, columns, primaryKey, sql: `SELECT ${columns.join(", ")} FROM ${sourceTable} ORDER BY ${primaryKey.join(", ")}` };
}
...
const rbac = [definition("portal_role_assignments", ["id", "username", "role", "disabled", "updated_at"], ["id"], "portal_users")] as const;
const policies = [
...
const audit = [definition("portal_audit_events", ["id", "created_at", "correlation_id", "actor_identity", "actor_role", "actor_groups_json", "action", "resource_type", "resource_id", "event_id", "schema_version", "approval_id", "run_id", "job_id", "outcome", "error_code", "metadata_json"], ["id"])] as const;

export const FULL_BACKUP_TABLES: ReadonlyArray<readonly [PortalBackupDomain, readonly FullBackupTableDefinition[]]> = [
  ["settings", settings], ["local-auth", localAuth], ["rbac", rbac], ["policies", policies], ["catalog", catalog], ["operations", operations], ["approvals", approvals], ["audit", audit],
];
...
function sameArray(left: readonly string[], right: readonly string[]): boolean { return left.length === right.length && left.every((item, index) => item === right[index]); }
function jsonScalar(value: unknown): boolean { return value === null || typeof value === "string" || typeof value === "boolean" || (typeof value === "number" && Number.isFinite(value)); }

export function validateFullBackupDomainPayload(domain: PortalBackupDomain, value: unknown): FullBackupDomainPayload {
  if (!plainObject(value) || Object.keys(value).some((key) => !["domain", "schemaVersion", "tables"].includes(key)) || value.domain !== domain || !Number.isSafeInteger(value.schemaVersion) || Number(value.schemaVersion) < 1 || !Array.isArray(value.tables)) throw new FullBackupValidationError();
  ...
  for (let index = 0; index < expected.length; index += 1) {
    ...
    if (!plainObject(table) || Object.keys(table).some((key) => !["name", "columns", "primaryKey", "rows"].includes(key)) || table.name !== descriptor.name || !Array.isArray(table.columns) || !Array.isArray(table.primaryKey) || !Array.isArray(table.rows)) throw new FullBackupValidationError();
    if (!sameArray(table.columns as string[], descriptor.columns) || !sameArray(table.primaryKey as string[], descriptor.primaryKey)) throw new FullBackupValidationError();
    ...
    for (const rawRow of table.rows) {
      if (!Array.isArray(rawRow) || rawRow.length !== descriptor.columns.length || rawRow.some((item) => !jsonScalar(item))) throw new FullBackupValidationError();
      const primaryValues = primaryIndexes.map((position) => rawRow[position]);
      if (primaryValues.some((item) => item === null || (typeof item === "string" && !item.length))) throw new FullBackupValidationError();
      const key = canonicalBackupJson(primaryValues);
...
async function loadTable(env: BackupExportEnv, descriptor: FullBackupTableDefinition): Promise<FullBackupTable> {
  ...
  try {
    ...
    const rows = sourceRows.map((row) => descriptor.columns.map((column) => {
      if (!Object.hasOwn(row, column) || typeof row[column] === "undefined") throw new BackupExportError("backup_schema_incompatible", 409, "Backup schema is incompatible");
      return row[column];
    }));
...
function domainExporter(domain: PortalBackupDomain, definitions: readonly FullBackupTableDefinition[]): FullBackupDomainExporter {
  return { domain, path: `domains/${domain}.json`, async export(env, schemaVersion) {
...
if (exporters.length !== PORTAL_BACKUP_DOMAINS.length || exporters.some((item, index) => item.domain !== PORTAL_BACKUP_DOMAINS[index])) throw new Error("Full backup exporter registry is not exhaustive or canonical");
export const FULL_BACKUP_EXPORTERS: ReadonlyMap<PortalBackupDomain, FullBackupDomainExporter> = new Map(exporters.map((item) => [item.domain, item]));
