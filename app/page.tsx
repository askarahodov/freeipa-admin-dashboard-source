  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {page === "users" && <Users items={filteredUsers} allGroups={directoryGroups} total={directoryUsers.length} source={directorySource} onCreate={() => setFreeIpaAction({ operation: "user_add", title: "Новый пользователь", preset: {} })} onAction={setFreeIpaAction} />}
        {page === "groups" && <Groups items={filteredGroups} allUsers={directoryUsers} source={directorySource} onCreate={() => setFreeIpaAction({ operation: "group_add", title: "Новая группа", preset: {} })} onAction={setFreeIpaAction} />}
...
function Users({ items, allGroups, total, source, onCreate, onAction }: { items: DirectoryUser[]; allGroups: DirectoryGroup[]; total: number; source: "demo" | "live" | "unconfigured"; onCreate: () => void; onAction: (action: FreeIpaAction) => void }) {
...
function AutomationCatalog({ items, mode, meta, error, loading, recentRuns, onSync, onOpenSettings, onLaunch }: { items: CatalogEvent[]; mode: IntegrationMode; meta: CatalogMeta; error: string; loading: boolean; recentRuns: RunRecord[]; onSync: () => void; onOpenSettings: () => void; onLaunch: (event: CatalogEvent) => void }) {
  ...
  return <div className="content-stack automation-page">
    ...
    {!loading && !items.length && <section className="panel catalog-empty"><strong>{error ? "Каталог не загружен" : "Процессы не найдены"}</strong><span>{error || "XYOps вернул пустой каталог либо текущий поиск не нашёл процессов."}</span><div className="empty-actions"><button className="secondary" onClick={onOpenSettings}>Открыть настройки</button><button className="primary" onClick={onSync}>Повторить синхронизацию</button></div></section>}
  </div>;
}
