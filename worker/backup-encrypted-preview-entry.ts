...
type BackupEncryptedPreviewAudit = (
  env: BackupExportEnv,
  context: AuditContext,
  event: AuditEventInput,
) => Promise<unknown>;

type BackupEncryptedPreviewSchemaInspector = (
  env: BackupExportEnv,
...
async function auditSafe(
  audit: BackupEncryptedPreviewAudit,
  env: BackupExportEnv,
  context: AuditContext,
  event: AuditEventInput,
): Promise<void> {
...
function normalizeError(error: unknown): BackupEncryptedPreviewError {
  ...
  if (error && typeof error === "object") {
    ...
    if (code && Number.isInteger(status) && status >= 400 && status <= 599) {
      const safeMessage = code === "backup_decryption_failed"
        ? "Backup decryption failed"
        : code === "backup_schema_incompatible"
          ? "Backup schema is incompatible"
          : "Encrypted backup preview failed";
      return new BackupEncryptedPreviewError(code, status, safeMessage);
    }
  }
  return new BackupEncryptedPreviewError("backup_encrypted_preview_failed", 500, "Encrypted backup preview failed");
}

export async function handleEncryptedBackupPreviewRequest(
  request: Request,
  env: BackupExportEnv,
  auditContext: AuditContext,
  dependencies: BackupEncryptedPreviewRouteDependencies = {},
): Promise<Response> {
  if (request.method !== "POST") return errorResponse(405, "backup_method_not_allowed", "Method not allowed");

  const declaredLength = Number(request.headers.get("content-length") ?? 0);
  if (Number.isFinite(declaredLength) && declaredLength > MAX_ENCRYPTED_PREVIEW_REQUEST_BYTES) {
    return errorResponse(413, "backup_request_too_large", "Encrypted backup preview request is too large");
  }
  ...
  try {
    ...
    if (new TextEncoder().encode(text).byteLength > MAX_ENCRYPTED_PREVIEW_REQUEST_BYTES) {
      throw new BackupEncryptedPreviewError("backup_request_too_large", 413, "Encrypted backup preview request is too large");
    }
    let input: unknown;
    try { input = JSON.parse(text); } catch {
      throw new BackupEncryptedPreviewError("backup_request_invalid", 400, "Encrypted backup preview request must contain valid JSON");
    }
    if (!plainObject(input)
        || Object.keys(input).some((key) => key !== "document" && key !== "password")
        || !Object.hasOwn(input, "document")
        || !Object.hasOwn(input, "password")) {
      throw new BackupEncryptedPreviewError("backup_request_invalid", 400, "Encrypted backup preview request contains invalid fields");
    }

    const rawDocument = input.document;
    if (plainObject(rawDocument) && plainObject(rawDocument.manifest) && Array.isArray(rawDocument.manifest.domains)) {
      domains = rawDocument.manifest.domains.filter((value): value is PortalBackupDomain => typeof value === "string");
    }
    if (plainObject(rawDocument) && plainObject(rawDocument.summary) && Number.isSafeInteger(rawDocument.summary.bytes)) {
      encryptedBytes = Number(rawDocument.summary.bytes);
    }
    ...
    if (schema.state !== "ready" || !Number.isSafeInteger(schema.currentVersion) || schema.currentVersion < 1) {
      throw new BackupEncryptedPreviewError("backup_schema_incompatible", 409, "Backup schema is incompatible");
    }

    const result = await (dependencies.previewBackup ?? previewEncryptedBackupImport)(
      env,
      rawDocument,
      input.password,
      schema,
