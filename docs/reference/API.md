...
`worker/integration-status-http.ts` owns `GET /api/integrations/status` after the existing local-session/security chain and resolves persisted FreeIPA/XYOps status from one atomic settings snapshot. `worker/operations-http-entry.ts` is the HTTP owner for the user-facing catalog routes above, the legacy `POST /api/integrations/actions` compatibility dispatch, run history/result-file reads, cancel/rerun, the two notification routes, and approval list/approve/reject/cancel/execute HTTP composition. Rerun and approval execution reuse the same operations-owned catalog-run handler; the claimed approval identifier is passed only server-side/in-memory. Approval-policy administration is owned by `worker/xyops-admin-http.ts`, and shared catalog normalization/cache helpers live in `worker/xyops-catalog-runtime.ts` rather than a central compatibility tail. Shared run synchronization/projection lives in `worker/xyops-run-runtime.ts`; XYOps continues to own upstream execution/scheduler semantics.
