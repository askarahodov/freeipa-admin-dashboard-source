...
const implementedSections = new Set(["general"]);

export function SettingsRouteShell({ title, description, children }: { title: string; description: string; children: ReactNode }) {
        ...
        <nav className={styles.sidebar} aria-label="Разделы настроек">
          {settingsSections.map((section) => {
            ...
            if (implementedSections.has(section.id)) {
              return <Link key={section.id} href={section.path} aria-current={active ? "page" : undefined} className={active ? styles.active : undefined}>{section.label}</Link>;
            }
