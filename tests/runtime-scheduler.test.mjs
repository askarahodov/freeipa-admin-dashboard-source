...
test("scheduler skips work while runtime is not ready", async () => {
  let runs = 0;
  const scheduler = createNonOverlappingScheduler({ intervalMs: 60_000, isReady: async () => false, run: async () => { runs += 1; } });
...
test("scheduler prevents overlapping executions", async () => {
  ...
  const scheduler = createNonOverlappingScheduler({ intervalMs: 60_000, isReady: async () => true, run: async () => blocked });
  const first = scheduler.runNow();
  await Promise.resolve();
  assert.deepEqual(await scheduler.runNow(), { status: "skipped", reason: "overlap" });
  release();
...
test("failed execution does not poison later runs", async () => {
  ...
  const scheduler = createNonOverlappingScheduler({
    intervalMs: 60_000,
    isReady: async () => true,
    run: async () => { attempts += 1; if (attempts === 1) throw new Error("upstream detail must not leak"); },
  });
...
test("stop clears pending timer and waits for active work", async () => {
  ...
  const scheduler = createNonOverlappingScheduler({
    ...
    isReady: async () => true,
    run: async () => blocked,
    setTimer(callback) { scheduledCallback = callback; return { id: 1 }; },
