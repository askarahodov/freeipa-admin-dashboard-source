...
const routeOperations = [
  ["user_add", "Создать пользователя"], ["user_enable", "Включить пользователя"], ["user_disable", "Отключить пользователя"], ["user_del", "Удалить пользователя"],
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {page === "automation" && <AutomationCatalog items={filteredCatalog} mode={catalogMode} meta={catalogMeta} loading={catalogLoading} recentRuns={recentRuns} onSync={() => void syncCatalog()} onLaunch={setSelectedProcess} />}
        {page === "users" && <Users items={filteredUsers} total={directoryUsers.length} source={directorySource} routes={routes} onCreate={() => setModal("user")} onAction={setTargetAction} />}
        {page === "groups" && <Groups items={filteredGroups} source={directorySource} routes={routes} onCreate={() => setModal("group")} onAction={setTargetAction} />}
...
function Users({ items, total, source, routes, onCreate, onAction }: { items: DirectoryUser[]; total: number; source: "demo" | "live" | "unconfigured"; routes: AutomationRoute[]; onCreate: () => void; onAction: (action: TargetAction) => void }) {
  const [filter, setFilter] = useState<"all" | "active" | "disabled">("all");
  const visible = items.filter((user) => filter === "all" || (filter === "active" ? user.active : !user.active));
  const enabled = new Set(routes.filter((route) => route.enabled).map((route) => route.operation));
  return <section className="panel table-panel section-page"><div className="panel-title"><div><h2>Пользователи FreeIPA</h2><p>{`${visible.length} из ${total} учетных записей · ${source === "live" ? "данные FreeIPA" : source === "demo" ? "демо-данные" : "FreeIPA не настроен"}`}</p></div><button className="primary" disabled={source === "unconfigured" || !enabled.has("user_add")} onClick={onCreate}>＋ Создать пользователя</button></div><div className="filter-row"><button className={`filter ${filter === "all" ? "active-filter" : ""}`} onClick={() => setFilter("all")}>Все</button><button className={`filter ${filter === "active" ? "active-filter" : ""}`} onClick={() => setFilter("active")}>Активные</button><button className={`filter ${filter === "disabled" ? "active-filter" : ""}`} onClick={() => setFilter("disabled")}>Отключённые</button></div><div className="data-table"><div className="tr th users-row"><span>Пользователь</span><span>Логин</span><span>Группы</span><span>Статус</span><span>Действия</span></div>{visible.map((u) => <div className="tr users-row" key={u.uid}><span className="person"><b>{u.name.split(" ").map(x => x[0]).join("")}</b><span><strong>{u.name}</strong><small>{u.email}</small></span></span><span className="mono">{u.uid}</span><span>{u.groups}</span><span><Status tone={u.active ? "success" : "neutral"}>{u.active ? "Активен" : "Отключён"}</Status></span><span className="row-actions">{enabled.has(u.active ? "user_disable" : "user_enable") && <button onClick={() => onAction({ operation: u.active ? "user_disable" : "user_enable", title: `${u.active ? "Отключить" : "Включить"} ${u.uid}`, preset: { username: u.uid, uid: u.uid, user: u.uid } })}>{u.active ? "Отключить" : "Включить"}</button>}{enabled.has("user_del") && <button className="danger-link" onClick={() => onAction({ operation: "user_del", title: `Удалить ${u.uid}`, preset: { username: u.uid, uid: u.uid, user: u.uid } })}>Удалить</button>}{!enabled.has("user_del") && !enabled.has(u.active ? "user_disable" : "user_enable") && <Status tone="neutral">Нет маршрута</Status>}</span></div>)}</div>{source === "unconfigured" && <div className="catalog-empty"><strong>FreeIPA не настроен</strong><span>Сохраните подключение в разделе «Настройки».</span></div>}</section>;
}

function Groups({ items, source, routes, onCreate, onAction }: { items: DirectoryGroup[]; source: "demo" | "live" | "unconfigured"; routes: AutomationRoute[]; onCreate: () => void; onAction: (action: TargetAction) => void }) {
  const enabled = new Set(routes.filter((route) => route.enabled).map((route) => route.operation));
  return <div className="content-stack"><div className="page-tools"><div><h2>Группы доступа</h2><p>{`${items.length} групп · ${source === "live" ? "данные FreeIPA" : source === "demo" ? "демо-данные" : "FreeIPA не настроен"}`}</p></div><button className="primary" disabled={source === "unconfigured" || !enabled.has("group_add")} onClick={onCreate}>＋ Создать группу</button></div>{source === "unconfigured" ? <section className="panel catalog-empty"><strong>FreeIPA не настроен</strong><span>Сохраните подключение в разделе «Настройки».</span></section> : <section className="group-grid">{items.map((g, i) => <article className="group-card" key={g.name}><div className={`group-avatar c${i % 4}`}>♣</div><h3>{g.name}</h3><p>{g.description}</p><div><span><strong>{g.members}</strong><small>участников</small></span><Status tone="violet">{g.type}</Status></div><div className="group-actions">{enabled.has("group_add_member") && <button onClick={() => onAction({ operation: "group_add_member", title: `Добавить участника в ${g.name}`, preset: { group: g.name, groupname: g.name } })}>＋ Участник</button>}{enabled.has("group_remove_member") && <button onClick={() => onAction({ operation: "group_remove_member", title: `Удалить участника из ${g.name}`, preset: { group: g.name, groupname: g.name } })}>− Участник</button>}{enabled.has("group_del") && <button className="danger-link" onClick={() => onAction({ operation: "group_del", title: `Удалить группу ${g.name}`, preset: { group: g.name, groupname: g.name } })}>Удалить</button>}{!["group_add_member", "group_remove_member", "group_del"].some((operation) => enabled.has(operation)) && <Status tone="neutral">Нет маршрутов управления</Status>}</div></article>)}</section>}</div>;
}

function AutomationCatalog({ items, mode, meta, loading, recentRuns, onSync, onLaunch }: { items: CatalogEvent[]; mode: IntegrationMode; meta: CatalogMeta; loading: boolean; recentRuns: RunRecord[]; onSync: () => void; onLaunch: (event: CatalogEvent) => void }) {
  ...
  return <div className="content-stack automation-page">
    <section className="automation-hero"><div><span className="eyebrow">XYOPS AUTOMATION PORTAL</span><h2>Портал автоматизаций</h2><p>Разделы, карточки и формы создаются из метаданных Events и Workflows. XYOps остаётся оркестратором, а портал предоставляет пользовательский интерфейс и визуализацию.</p></div><button className="secondary" disabled={loading} onClick={onSync}>{loading ? "Синхронизация…" : "⟳ Обновить каталог"}</button></section>
    <section className={`catalog-sync ${meta.stale ? "stale" : ""}`}><span className={`source-dot ${mode}`} /><div><strong>{meta.source === "xyops" ? "Каталог синхронизирован с XYOps" : meta.source === "cache" ? "Показан сохранённый снимок" : meta.source === "demo" ? "Демонстрационный каталог" : "XYOps не настроен"}</strong><small>{meta.syncedAt ? `Последняя успешная синхронизация: ${new Date(meta.syncedAt).toLocaleString("ru-RU")}` : "Снимок каталога ещё не создан"}</small></div>{meta.changes.length > 0 && <Status tone="violet">{meta.changes.filter((change) => change.kind === "new").length} новых · {meta.changes.filter((change) => change.kind === "changed").length} изменено · {meta.changes.filter((change) => change.kind === "removed").length} удалено</Status>}{meta.stale && <Status tone="warning">XYOps временно недоступен</Status>}</section>
...
function Settings({ routes, catalog, catalogLoading, onSync, onRoutesChange, notify }: { routes: AutomationRoute[]; catalog: CatalogEvent[]; catalogLoading: boolean; onSync: () => void; onRoutesChange: (routes: AutomationRoute[]) => void; notify: (message: string) => void }) {
  ...
  function addRoute() {
    ...
    const route: AutomationRoute = { key: baseKey || `${operation}-${routes.length + 1}`, title: routeTitle.trim() || selectedEvent.title, operation, eventId: selectedEvent.id, kind: selectedEvent.kind, enabled: true, targets: selectedEvent.targets, fields: selectedEvent.fields };
...
function GeneratedFields({ fields, eventId, preset = {} }: { fields: RouteField[]; eventId: string; preset?: Record<string, string> }) {
  ...
  const sections = visibleFields.reduce<Array<{ title: string; fields: RouteField[] }>>((result, field) => {
    const title = field.section || "Параметры";
    const current = result.find((section) => section.title === title);
    if (!current) result.push({ title, fields: [field] }); else current.fields.push(field);
    return result;
  }, []);
  ...
  }}>{sections.map((section, index) => <section className="generated-section" key={`${section.title}-${index}`}>{(sections.length > 1 || section.title !== "Параметры") && <div className="generated-section-title"><strong>{section.title}</strong><span>{section.fields.length} полей</span></div>}<div className="dynamic-fields">{section.fields.map((field) => <DynamicField field={field} eventId={eventId} preset={preset} key={field.key} />)}</div></section>)}</div>;
}
...
function DynamicField({ field, eventId, preset = {} }: { field: RouteField; eventId: string; preset?: Record<string, string> }) {
  ...
  if (field.type === "select") return <label>{field.label}{field.required && <em>обязательно</em>}<select name={field.key} required={field.required} defaultValue={String(defaultValue ?? field.options?.[0] ?? "")}><option value="" disabled>Выберите значение</option>{(field.options ?? []).map((option) => <option value={option} key={option}>{option}</option>)}</select><small>{field.description || `${field.key} → ${field.target ?? "params"}`}</small></label>;
