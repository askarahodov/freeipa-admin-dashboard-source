...
Phase #631 checkpoint A consolidated the former `freeipa-group-member-entry.ts -> freeipa-user-bulk-entry.ts -> freeipa-user-query-entry.ts` wrapper chain into one `freeipa-http-entry.ts` compatibility owner at the same historical position. Checkpoint B1 moves the base FreeIPA users/groups response logic into `freeipa-base-read.ts` and the upstream RPC transport into `freeipa-rpc.ts`, while `worker/index.ts` keeps only the compatibility delegation after it has resolved the existing effective integration settings. Query/filter/pagination, CSV export, bulk orchestration, group-member projection and the legacy status-permission projection keep the same downstream auth/permission behavior. `POST /api/integrations/freeipa/actions` deliberately remains in `worker/index.ts` for B2 because its audit and operation-run persistence are shared with the central integration runtime.
