...
Integration status/audit and the legacy direct settings update still reach the compatibility integration handler, while #633 checkpoint A moves settings read and connection-test HTTP ownership to `worker/settings-lifecycle-entry.ts` behind the unchanged auth/session boundary.
...
| `GET` | `/api/integrations/settings` | Read public/redacted active integration settings. | `settings.manage` + administrator authorization; owner `worker/settings-lifecycle-entry.ts` |
...
| `POST` | `/api/integrations/settings/test` | Test FreeIPA or XYOps draft/current integration settings. | `settings.manage` + administrator authorization; owner `worker/settings-lifecycle-entry.ts` |
...
| --- | --- | --- | --- | --- |
| `GET` | `/api/integrations/users` | List FreeIPA users; optional query/pagination layer recognizes `q,status,group,sort,direction,page,pageSize`. | `directory.read` through current integration/session boundary | `worker/freeipa-http-entry.ts` |
...
| `GET` | `/api/integrations/groups` | List FreeIPA groups. | `directory.read` | `worker/freeipa-http-entry.ts` |
...
| `POST` | `/api/integrations/freeipa/actions` | Single supported FreeIPA mutation selected by operation. | `freeipa.write`; `freeipa.delete` for `user_del`/`group_del` | `worker/freeipa-http-entry.ts` |
...
`worker/operations-http-entry.ts` is the HTTP owner for the user-facing catalog routes above, the legacy `POST /api/integrations/actions` compatibility dispatch, run history/result-file reads, cancel/rerun, the two notification routes, and approval list/approve/reject/cancel/execute HTTP composition. Rerun and approval execution reuse the same operations-owned catalog-run handler; the claimed approval identifier is passed only server-side/in-memory. Approval-policy administration remains in `worker/index.ts`, and shared catalog normalization/cache helpers are reused rather than forked. Shared run synchronization/projection lives in `worker/xyops-run-runtime.ts`; XYOps continues to own upstream execution/scheduler semantics.
