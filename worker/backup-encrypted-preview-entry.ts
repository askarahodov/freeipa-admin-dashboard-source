...
function normalizeError(error: unknown): BackupEncryptedPreviewError {
  if (error instanceof BackupEncryptedPreviewError) {
    ...
    return safe
      ? new BackupEncryptedPreviewError(error.code, safe.status, safe.message)
      : new BackupEncryptedPreviewError(
        "backup_encrypted_preview_failed",
        500,
  ...
  return new BackupEncryptedPreviewError(
    "backup_encrypted_preview_failed",
    500,
    "Encrypted backup preview failed",
  );
...
function allowlistedDomains(value: unknown): PortalBackupDomain[] {
...
export async function handleEncryptedBackupPreviewRequest(
  ...
  if (Number.isFinite(declaredLength) && declaredLength > MAX_ENCRYPTED_PREVIEW_REQUEST_BYTES) {
    return errorResponse(
      ...
      "Encrypted backup preview request is too large",
    );
  }
  ...
  try {
    ...
    if (new TextEncoder().encode(text).byteLength > MAX_ENCRYPTED_PREVIEW_REQUEST_BYTES) {
      throw new BackupEncryptedPreviewError(
        "backup_request_too_large",
        413,
        "Encrypted backup preview request is too large",
      );
    ...
    try {
      ...
      throw new BackupEncryptedPreviewError(
        ...
        400,
        "Encrypted backup preview request must contain valid JSON",
      );
    }
    if (!plainObject(input)
        || Object.keys(input).some((key) => !["document", "password", "domains"].includes(key))
        || !Object.hasOwn(input, "document")
        || !Object.hasOwn(input, "password")) {
      throw new BackupEncryptedPreviewError(
        "backup_request_invalid",
        400,
        "Encrypted backup preview request contains invalid fields",
      );
    }

    const rawDocument = input.document;
    domains = Object.hasOwn(input, "domains")
      ? allowlistedDomains(input.domains)
      : plainObject(rawDocument)
        && plainObject(rawDocument.manifest)
        && Array.isArray(rawDocument.manifest.domains)
        ? allowlistedDomains(rawDocument.manifest.domains)
        : [];
    if (plainObject(rawDocument)
        && plainObject(rawDocument.summary)
        && Number.isSafeInteger(rawDocument.summary.bytes)) {
    ...
    const result = await (dependencies.previewBackup ?? previewEncryptedBackupImport)(
      ...
      {},
      { selectedDomains: Object.hasOwn(input, "domains") ? input.domains : undefined },
    );
