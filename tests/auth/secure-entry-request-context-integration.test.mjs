...
test("secure entry reuses canonical identity and correlation only for audit metadata", async () => {
  ...
  assert.match(source, /import runtime from "\.\/backup-http-entry\.ts";/u);
  const backupOwner = await readFile(new URL("../../worker/backup-http-entry.ts", import.meta.url), "utf8");
  assert.match(backupOwner, /import runtime from "\.\/freeipa-http-entry\.ts";/u);
