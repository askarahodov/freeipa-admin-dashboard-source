...
export async function validateEncryptedBackupDocument(value: unknown): Promise<ValidatedEncryptedBackupDocument> {
  ...
  if (Object.keys(value).some((key) => !["manifest", "payloads", "summary"].includes(key))
      || !Object.hasOwn(value, "manifest") || !Object.hasOwn(value, "payloads") || !Object.hasOwn(value, "summary")) {
  ...
  if (!arraysEqual(manifest.domains, canonicalDomains(manifest.domains)) || manifest.entries.length !== manifest.domains.length) {
  ...
  for (let index = 0; index < manifest.domains.length; index += 1) {
    ...
    if (entry.bytes !== envelopeBytes || entry.sha256 !== checksum) fail("backup_corrupted", 422, "Encrypted backup integrity check failed");
  ...
  for (const path of Object.keys(payloads)) {
    if (!expectedPaths.has(path)) fail("backup_payload_unexpected", 422, "Encrypted backup contains an unexpected payload");
  }
  ...
  return {
    ...
    summary: { entries: Number(value.summary.entries), records: Number(value.summary.records), bytes: Number(value.summary.bytes) },
  };
}
