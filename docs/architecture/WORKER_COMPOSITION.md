...
#633 checkpoint A moves `settings.read` and `settings.test` into `worker/settings-http.ts`, invoked from `secure-entry.ts` only after canonical identity normalization; lifecycle effective/draft calls reach the same owner through that boundary, preserving local-admin/service-admin ordering. The legacy direct `PUT /api/integrations/settings`, source/reset compensation flow and revision wrapper remain separate for the next bounded #633 slices; no second settings source of truth is introduced.
...
| Settings | canonical permissions + route contract | #633A: `settings-http.ts` owns active-settings read/connection-test after `secure-entry.ts`; `settings-lifecycle-entry.ts` retains effective/draft lifecycle; source/revision adapters retain their existing responsibilities; the legacy direct `PUT /api/integrations/settings` compatibility write remains downstream for the next bounded slice | `app_settings`, settings drafts/revisions/apply/reset/source-lock state; encrypted integration secrets | settings lifecycle/source/revision tests; settings audit/compensation events |
