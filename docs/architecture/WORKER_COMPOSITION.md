...
| XYOps/run/approval | canonical route metadata plus portal permissions | `operations-http-entry.ts` owns run reads/result-file proxy/cancel/rerun/notifications and approval HTTP composition; catalog execution and approval-policy administration stay in `worker/index.ts` | portal operation/catalog/approval tables; server-side XYOps client; XYOps owns execution semantics | `tests/operations/**`, `tests/xyops/**`, route-owner contracts, audit events from current handlers |
