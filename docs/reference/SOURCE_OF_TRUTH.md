...
| Production process/runtime ownership | `Dockerfile`, `scripts/start-production.mjs`, `runtime/production-runtime.mjs`, runtime host/database/scheduler/shutdown modules | `architecture/ARCHITECTURE.md`, `architecture/PROJECT_STRUCTURE.md`, `reference/CONFIGURATION.md` | `scripts/start-worker.mjs` и Wrangler dev mode не являются canonical production process owner после #51/#194 |
| Canonical database schema | `db/portal-schema.ts` | `operations/DATABASE_MIGRATIONS.md` | Итоговый inventory должен определяться кодом, а не вручную переписанной таблицей в docs |
| Versioned schema migrations | migration registry/modules under `db/`, migration journal runtime | `operations/DATABASE_MIGRATIONS.md` | Выпущенные migration definitions/checksums immutable |
| Schema startup/adoption/drift behavior | schema migration runtime modules and tests | `operations/DATABASE_MIGRATIONS.md` | Issue #57 может описывать ещё не завершённые этапы и не переопределяет runtime |
...
| Storage read-only status | storage status contract/handler | `operations/STORAGE_STATUS.md` | Endpoint не является migration или repair API |
| Storage integrity | integrity contract/handler and canonical index registry | `operations/STORAGE_INTEGRITY.md` | Read-only diagnostics не должны превращаться в arbitrary SQL/repair surface |
| Backup/restore | backup/restore runtime contracts and recovery scripts | профильные backup/restore/recovery runbook | Ownership placement and repository boundaries are mapped in `architecture/PROJECT_STRUCTURE.md`; exact destructive procedure remains in the dedicated runbook |
...
| Worker application composition | `worker/application-composition-contract.ts` для build/schema/application/scheduled/framework owner metadata и bounded compatibility exceptions; фактические imports/build config остаются executable evidence | `architecture/ARCHITECTURE.md`, `architecture/PROJECT_STRUCTURE.md`, `architecture/WORKER_COMPOSITION.md` | Central `worker/index.ts` tail removed in #635; framework/static/RSC/image owner is `worker/framework-http-entry.ts`; #636 guards against regressions and new untracked wrappers |
...
Фактический dispatch, middleware order, request adaptation/validation, maintenance/schema exceptions и response behavior подтверждаются handler/tests. `worker/application-composition-contract.ts` является pure metadata owner только для верхнеуровневой composition ownership и bounded compatibility exceptions; [`../architecture/WORKER_COMPOSITION.md`](../architecture/WORKER_COMPOSITION.md) даёт подробную current-state map. До parity cutover нельзя трактовать route metadata или composition metadata как runtime enforcement.
...
`architecture/PROJECT_STRUCTURE.md` и `architecture/ARCHITECTURE.md` дают текущую ownership/boundary map. Однако код исторически развивался через root/shared modules и Worker wrappers, поэтому exact behavior всё ещё нужно проверять по текущему owner-code/tests; documentation map не является разрешением создавать параллельные abstractions.
...
## #635 final Worker composition checkpoint

The historical central `worker/index.ts` compatibility tail is removed. Top-level composition ownership is machine-readable in `worker/application-composition-contract.ts`; framework/static/RSC/image behavior is owned by `worker/framework-http-entry.ts` / `worker/framework-http.ts`. Remaining compatibility adapters are explicitly enumerated with reasons and removal conditions and are not parallel route registries.
