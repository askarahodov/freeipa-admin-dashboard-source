...
Node consumes `NODE_USE_ENV_PROXY` and installs its built-in proxy handling during process startup, before application modules run. The portal then performs its own fail-closed preflight before Worker loading, Gateway startup, application startup, or scheduler activity. The supported path affects Node's default global HTTP(S) agents and global `fetch` dispatcher, so the existing outbound FreeIPA Gateway `fetch` and XYOps `fetch` paths participate without changing their request/response, authentication, timeout, redirect, or TLS contracts. The portal deliberately does not depend on the newer runtime-only `http.setGlobalProxyFromEnv()` API, which is unavailable on otherwise supported Node 22 proxy releases.
