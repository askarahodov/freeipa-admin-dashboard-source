...
`application.ts` classifies requests through `application-router.ts`, which reuses the canonical matcher from `src/auth/portal-route-router.ts`; it does not define a second route registry. The router exposes four explicit dispatch registrations: `stable`, `negative`, `supplemental` and `framework`. At the current migration checkpoint all four registrations still delegate through `maintenance-mode-root-entry.ts`, so the compatibility runtime continues to receive the original request, environment and context and remains authoritative for maintenance, authentication, authorization and stable-route handlers. The registrations are the bounded replacement seam for later route-family cutovers, not a second per-route source of truth.
...
Run the narrow worker/domain contract tests for the changed boundary. `tests/auth/portal-application-router.test.mjs` guards canonical route coverage, route-inventory-to-registration dispatch, method/unknown classification, unchanged request/environment/context delegation, security-preserving negative response finalization and the explicit composition placement. Auth, permission, mutation or browser-observable routing changes normally require the relevant Auth E2E scope. Recovery/storage/health changes require their profile contract tests and runbook review.
