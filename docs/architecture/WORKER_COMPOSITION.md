...
#635 B1 extracts static/RSC/application hosting behavior from `worker/index.ts`: `framework-http.ts` owns the route-to-root HTML compatibility decision and fallback policy, while `framework-http-entry.ts` owns Vinext image optimization and app-router integration. The application router still classifies framework/image traffic without bypassing the compatibility chain in this checkpoint; direct application-level dispatch is the next parity-gated step.
