...
export default function FreeIpaUserBrowser() {
  ...
  return createPortal(
    ...
    <div className="freeipa-user-browser-shell">
      ...
      <form className="freeipa-user-query" onSubmit={(event) => { event.preventDefault(); setFilter({ q: draft.trim() }); }}><div className="ds-field"><label className="ds-field-label">Поиск</label><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Логин, ФИО, email или группа" maxLength={160} /></div><div className="ds-field"><label className="ds-field-label">Состояние</label><select value={query.status} onChange={(event) => setFilter({ status: event.target.value as FreeIpaUserStatus })}><option value="all">Все</option><option value="active">Активные</option><option value="disabled">Отключённые</option></select></div><div className="ds-field"><label className="ds-field-label">Группа</label><select value={query.group} onChange={(event) => setFilter({ group: event.target.value })}><option value="">Все группы</option>{groups.map((group) => <option value={group} key={group}>{group}</option>)}</select></div><div className="ds-field"><label className="ds-field-label">Сортировка</label><select value={query.sort} onChange={(event) => setFilter({ sort: event.target.value as FreeIpaUserSort })}><option value="uid">Логин</option><option value="name">Имя</option><option value="email">Email</option><option value="groups">Количество групп</option><option value="status">Статус</option></select></div><div className="ds-field"><label className="ds-field-label">На странице</label><select value={query.pageSize} onChange={(event) => setFilter({ pageSize: Number(event.target.value) })}>{[10, 25, 50, 100].map((size) => <option value={size} key={size}>{size}</option>)}</select></div><button className="secondary freeipa-sort-direction" type="button" onClick={() => setFilter({ direction: query.direction === "asc" ? "desc" : "asc" })}>{query.direction === "asc" ? "↑ По возрастанию" : "↓ По убыванию"}</button><button className="primary" type="submit">Найти</button><div className="ds-field"><button className="secondary" type="button" onClick={() => { setDraft(""); setQuery({ ...defaultQuery, pageSize: query.pageSize }); }}>Сбросить</button></div></form>
