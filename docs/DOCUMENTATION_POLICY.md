...
Документация FreeIPA Admin Dashboard является частью инженерного контракта проекта. Изменение считается незавершённым, если затронутое фактическое поведение, архитектура, API, security boundary, конфигурация, схема данных, deployment или runbook изменились, а документация осталась прежней.
