  ...
  return <>
    ...
    {confirmation && <div className="portal-confirm-backdrop" role="presentation" onMouseDown={() => setConfirmation(null)}>
      <section className={`portal-confirm-dialog ${confirmation.tone}`} role="alertdialog" aria-modal="true" aria-labelledby="portal-confirm-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {confirmation.requireReason && <div className="ds-field"><label htmlFor="portal-confirm-reason">Причина отклонения</label><textarea id="portal-confirm-reason" className="ds-textarea" autoFocus value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Опишите причину для инициатора и аудита" /><span className="ds-field-helper">Минимум 3 символа</span></div>}
        {confirmation.requireDeletePhrase && <div className="ds-field"><label htmlFor="portal-confirm-delete-phrase">Контрольная фраза</label><input id="portal-confirm-delete-phrase" className="ds-input" autoFocus value={deletePhrase} onChange={(event) => setDeletePhrase(event.target.value)} placeholder="УДАЛИТЬ" autoComplete="off" /><span className="ds-field-helper">Введите УДАЛИТЬ заглавными или строчными буквами</span></div>}
