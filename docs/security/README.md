...
`SECURITY_MODEL.md` and operational recovery/security runbooks remain at their current canonical paths until their own explicit #268 migration slice. The former root `docs/LOCAL_AUTH_RBAC.md` path is retained only as a relocation pointer for existing external links.
