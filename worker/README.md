...
Phase #631 checkpoint A consolidates the former `freeipa-group-member-entry.ts -> freeipa-user-bulk-entry.ts -> freeipa-user-query-entry.ts` wrapper chain into one `freeipa-http-entry.ts` compatibility owner at the same historical position. Query/filter/pagination, CSV export, bulk orchestration, group-member projection and the legacy status-permission projection therefore keep the same downstream auth/permission/audit behavior while removing two wrapper layers. Encrypted backup and import-preview predispatch were moved upward into `backup-selective-restore-root-entry.ts`, preserving their existing order without leaving backup ownership inside the FreeIPA adapter. Base FreeIPA users/groups/actions still live in `worker/index.ts` at this checkpoint; moving those handlers is a separate bounded #631 slice because they share RPC/settings/operation-run/audit helpers with the central integration runtime.
