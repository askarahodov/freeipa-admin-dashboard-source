...
test("FreeIPA domain modules have one canonical implementation path", async () => {
  ...
  assert.equal(
    ...
    "root FreeIPA UI events file must remain a compatibility-only re-export until its final removal",
  );
});
...
test("tests use canonical FreeIPA UI events source instead of the temporary root shim", async () => {
  const files = await collectSourceFiles("tests");
  const violations = [];

  for (const relativePath of files) {
    if (relativePath === "tests/freeipa-domain-path-contract.test.mjs") continue;
    const content = await readFile(path.join(repoRoot, relativePath), "utf8");
    if (/\.\.\/freeipa-ui-events(?:\.ts)?["']/.test(content)) violations.push(relativePath);
  }

  assert.deepEqual(violations, [], `tests still depend on the temporary FreeIPA UI events root shim: ${violations.join(", ")}`);
});
