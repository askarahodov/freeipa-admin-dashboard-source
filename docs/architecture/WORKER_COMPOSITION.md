...
The graph is not strictly linear. `settings-source-safe-entry.ts` chooses either the source-aware path or the lifecycle path depending on the request and may construct an effective integration environment before delegation. `secure-entry.ts` normalizes the accepted workspace/proxy/static identity context before delegating to `freeipa-http-entry.ts`. That adapter owns the complete FreeIPA HTTP surface and falls through unmatched requests to `operations-http-entry.ts`. #632 checkpoint A lets that post-security operations adapter own run-history/result-file reads and per-identity notifications before unmatched traffic continues to `worker/index.ts`; neither adapter is a second authentication boundary.
...
Checkpoint #632A moves `xyops.runs.list`, `xyops.runs.file`, `xyops.notifications.list` and `xyops.notifications.read` into `worker/operations-http-entry.ts`. Shared run status/synchronization/projection logic lives in `worker/xyops-run-runtime.ts` so the still-central cancel/rerun/catalog mutation paths reuse one implementation. Catalog execution, run mutations and approvals remain in `worker/index.ts` for later bounded #632 slices. XYOps remains authoritative for process definitions and execution/scheduler/queue semantics.
...
| XYOps/run/approval | canonical route metadata plus portal permissions | `operations-http-entry.ts` owns run reads/result-file proxy/notifications; remaining catalog/run mutations/approvals stay in `worker/index.ts` | portal operation/catalog/approval tables; server-side XYOps client; XYOps owns execution semantics | `tests/operations/**`, `tests/xyops/**`, route-owner contracts, audit events from current handlers |
...
12. `operations-http-entry.ts` owns #632A run-history/result-file reads and notifications after the same security boundary; unmatched traffic falls through to `worker/index.ts`;
