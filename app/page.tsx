...
const processIconGlyphs: Record<string, string> = { event: "▶", workflow: "⌘", database: "▤", backup: "▣", server: "▥", user: "♙", group: "♣", security: "◇", network: "⌁", settings: "⚙", storage: "▦", deploy: "↥", report: "≣" };
function processIconGlyph(icon: string | undefined, kind: "event" | "workflow"): string {
  if (!icon) return processIconGlyphs[kind];
  return processIconGlyphs[icon] ?? icon.slice(0, 2).toUpperCase();
}
...
export default function Home() {
  ...
  const automationSections = useMemo<AutomationSection[]>(() => Array.from(new Set(catalog.map((event) => event.category || "general"))).map((category) => {
    ...
    return { category, slug: automationSlug(category), count: items.length, events: items.filter((event) => event.kind === "event").length, workflows: items.filter((event) => event.kind === "workflow").length, order: Math.min(...items.map((event) => event.order ?? 0)) };
