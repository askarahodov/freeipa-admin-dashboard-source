  ...
  return createPortal(
    <section className="panel settings-lifecycle" data-testid="settings-lifecycle-wizard">
      <div className="settings-lifecycle-head">
        ...
        <div className="settings-revision"><small>Активная revision</small><strong>{effective?.revision || "ENV"}</strong><button className="ds-btn ds-btn-secondary" disabled={Boolean(busy)} onClick={() => void load()}>Обновить</button></div>
      </div>
      ...
      {!effective ? <div className="catalog-empty"><strong>{busy === "load" ? "Загрузка настроек…" : "Настройки не загружены"}</strong></div> : <>
        ...
        <div className="settings-lifecycle-grid">
          {fieldVisible("demoMode") && <section><div className="settings-lifecycle-title"><h3>Общие</h3><SourceControl name="demoMode" field={effective.fields.demoMode} selected={resetSet.has("demoMode")} disabled={disabled} onToggle={toggleReset} /></div><label className={`ds-checkbox ${resetSet.has("demoMode") ? "reset-pending" : ""}`}><input type="checkbox" checked={form.demoMode} disabled={disabled || resetSet.has("demoMode")} onChange={(event) => updateField("demoMode", event.target.checked)} /><span><strong>Демо-режим</strong></span></label><span className="ds-field-helper">{resetSet.has("demoMode") ? `После применения: ${effective.fields.demoMode.fallbackSource === "environment" ? "ENV" : "DEFAULT"}` : "Не выполнять реальные вызовы FreeIPA и XYOps"}</span></section>}
          {(["ipaUrl", "ipaUsername", "ipaPassword"] as SettingField[]).some(fieldVisible) && <section>
            ...
            {fieldVisible("ipaUrl") && <div className="ds-field"><label className="ds-field-label">Адрес сервера</label><input className="ds-input" value={form.ipaUrl} disabled={disabled || resetSet.has("ipaUrl")} onChange={(event) => updateField("ipaUrl", event.target.value)} /></div>}
            {fieldVisible("ipaUsername") && <div className="ds-field"><label className="ds-field-label">Service account</label><input className="ds-input" value={form.ipaUsername} disabled={disabled || resetSet.has("ipaUsername")} onChange={(event) => updateField("ipaUsername", event.target.value)} /></div>}
            {fieldVisible("ipaPassword") && <div className="ds-field"><label className="ds-field-label">Новый пароль<SourceControl name="ipaPassword" field={effective.fields.ipaPassword} selected={resetSet.has("ipaPassword")} disabled={disabled} onToggle={toggleReset} /></label><input className="ds-input" type="password" value={form.ipaPassword} disabled={disabled || resetSet.has("ipaPassword")} onChange={(event) => updateField("ipaPassword", event.target.value)} placeholder={effective.settings.freeipa.passwordConfigured ? "Сохранён — заполните только для замены" : "Не настроен"} autoComplete="new-password" /></div>}
          </section>}
          {(["xyopsUrl", "xyopsApiKey"] as SettingField[]).some(fieldVisible) && <section>
            <div className="settings-lifecycle-title"><h3>XYOps</h3></div>
            {fieldVisible("xyopsUrl") && <div className="ds-field"><label className="ds-field-label">Адрес XYOps</label><input className="ds-input" value={form.xyopsUrl} disabled={disabled || resetSet.has("xyopsUrl")} onChange={(event) => updateField("xyopsUrl", event.target.value)} /></div>}
            {fieldVisible("xyopsApiKey") && <div className="ds-field"><label className="ds-field-label">Новый API key<SourceControl name="xyopsApiKey" field={effective.fields.xyopsApiKey} selected={resetSet.has("xyopsApiKey")} disabled={disabled} onToggle={toggleReset} /></label><input className="ds-input" type="password" value={form.xyopsApiKey} disabled={disabled || resetSet.has("xyopsApiKey")} onChange={(event) => updateField("xyopsApiKey", event.target.value)} placeholder={effective.settings.xyops.apiKeyConfigured ? "Сохранён — заполните только для замены" : "Не настроен"} autoComplete="new-password" /></div>}
          </section>}
        </div>
