...
test("documentation navigation exposes the normalized reference layer as current state", async () => {
  const [index, inventory, ai] = await Promise.all([read("docs/README.md"), read("docs/DOCUMENTATION_INVENTORY.md"), read("docs/ai/README.md")]);
...
test("permissions reference tracks the canonical built-in permission order", async () => {
  ...
  assert.ok(codes.includes("backup.restore.preview")); assert.match(reference, /backup\.restore\.preview/);
  assert.match(reference, /canonical|канонич/iu); assert.doesNotMatch(reference, /backup\.restore\.preview[^\n]{0,180}(not canonical|not promoted|orphan)/iu);
});

test("API reference tracks known literal storage route constants", async () => {
  const [api, status, integrity, preflight, apply] = await Promise.all([read("docs/reference/API.md"), read("src/storage/status/storage-status-contract.ts"), read("src/storage/integrity/storage-integrity-contract.ts"), read("src/storage/migration/preflight/storage-migration-preflight-contract.ts"), read("src/storage/migration/apply/storage-migration-apply-contract.ts")]);
...
test("configuration reference separates operator settings from ephemeral Gateway credentials", async () => {
  const [example, configuration] = await Promise.all([read(".env.example"), read("docs/reference/CONFIGURATION.md")]);
  for (const name of ["ADMIN_TOKEN", "CONFIG_ENCRYPTION_KEY", "PORTAL_IDENTITY_MODE", "PORTAL_BOOTSTRAP_ADMIN_PASSWORD", "IPA_URL", "IPA_USERNAME", "IPA_PASSWORD", "XYOPS_URL", "XYOPS_API_KEY"]) {
    assert.ok(example.includes(`${name}=`), `.env.example no longer documents ${name}`); assert.match(configuration, new RegExp(`\\\`${name}\\\``));
  }
...
test("error reference has no duplicate table-row machine codes", async () => {
  const errors = await read("docs/reference/ERROR_CODES.md"); const codes = tableCodes(errors);
  const duplicates = codes.filter((code, index) => codes.indexOf(code) !== index); assert.deepEqual([...new Set(duplicates)], []);
  for (const code of ["health_live", "dependencies_degraded", "storage_status_unavailable", "storage_integrity_forbidden", "migration_preflight_request_invalid", "maintenance_transition_failed", "backup_restore_commit_failed", "settings_revision_conflict"]) assert.ok(errors.includes(`\`${code}\``), `missing stable machine code: ${code}`);
