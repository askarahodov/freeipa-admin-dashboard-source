...
- the pre-schema **pass-through** for liveness/readiness, legacy integration health, dependency health, the sanitized health diagnostics document/assets and health metrics;
...
`worker/application.ts` is the single HTTP application composition point after that entry boundary. `application-router.ts` classifies each request from canonical route metadata plus explicit supplemental infrastructure classifications. Public/infrastructure health classifications are offered to `worker/health-http.ts` first. Non-health classifications continue into `worker/security-composition.ts` exactly as before the #630 pilot.
...
schema-migrations-entry.ts
  ...
  -> application-router.ts
       ...
       |       -> health-diagnostics-ui.ts
       |       `-> health-metrics.ts
       `-> non-health classification
            ...
            -> settings-source-safe-entry.ts
                 ...
                 |                                     v
                 `-> settings-lifecycle-entry.ts -> secure-entry.ts -> freeipa-http-entry.ts -> worker/index.ts
```

The graph is not strictly linear. `settings-source-safe-entry.ts` chooses either the source-aware path or the lifecycle path depending on the request and may construct an effective integration environment before delegation. `secure-entry.ts` resolves the existing proxy/static/workspace request identity and then delegates the secured request/environment into `freeipa-http-entry.ts`; this placement makes the FreeIPA adapter an HTTP/domain owner, not a new authentication boundary. Routes outside the FreeIPA surface continue through that adapter to `worker/index.ts`. Future migration must preserve these branch behaviors until explicit replacements have parity evidence.

`worker/index.ts` remains a broad compatibility/base HTTP owner for settings, XYOps/operations and framework hosting, plus the sanitized backup export entry, Vinext image optimization, route-to-root HTML adaptation for the SPA-like administrative screens and the final Vinext application/static fallback. It no longer owns FreeIPA HTTP routes; it reuses the shared portal-access and operation-run runtimes for the remaining central compatibility behavior.
...
`tests/auth/portal-application-router.test.mjs` materializes every current canonical route pattern and proves that the explicit application router resolves it back to the same route id. It also proves that non-health negative response finalization cannot promote successful, authentication, authorization, conflict, rate-limit or maintenance responses into routing errors. `tests/runtime/health-http-owner.test.mjs` proves the separate health branch and its pre-schema availability/parity. `tests/freeipa/freeipa-http-owner.test.mjs` proves the single FreeIPA HTTP owner, retired wrapper files, absence of the central mutation fallback/duplicate helpers, backup ownership separation and route-owner metadata.

### Infrastructure and schema

`health.live`, `health.ready`, `integration.health.compat`, `schema.status`

Actual health HTTP owner: `worker/health-http.ts`, delegating stable behavior to `worker/health-contracts.ts`. `worker/schema-migrations-entry.ts` owns only the pre-schema pass-through for those health classifications. `schema.status` remains owned and authorized at the schema boundary. Liveness/readiness intentionally execute before ordinary schema readiness and before non-health security composition.
...
Phase #631 now has one compatibility HTTP owner, `worker/freeipa-http-entry.ts`, for base users/groups reads, query/filter projection, CSV export, bulk orchestration, group-member projection, mutation actions and the status permission projection. The adapter executes after `secure-entry.ts` identity normalization. Canonical query/domain logic remains under `src/freeipa/**`; route-neutral mutation normalization is in `worker/freeipa-action-runtime.ts`, reusable Gateway/direct transport in `worker/freeipa-rpc.ts`, persisted settings resolution in `worker/integration-settings-runtime.ts`, and shared access/run helpers in `worker/portal-access-runtime.ts` and `worker/operation-run-runtime.ts`. `worker/index.ts` no longer contains a FreeIPA HTTP action route or a second FreeIPA action implementation. The private Gateway remains the upstream trust boundary and FreeIPA credentials stay server-only.
...
HTTP ownership is distributed across the backup entry/dispatch adapters. `worker/backup-selective-restore-root-entry.ts` owns encrypted-backup/import-preview predispatch after selective-restore dispatch and delegates non-backup traffic to `session-management-entry.ts`; FreeIPA is no longer in that backup ancestry. Canonical backup/recovery domain logic remains under `src/backup/**` and `src/recovery/**`. Destructive full restore is intentionally offline and is not represented as a normal HTTP route.
...
| FreeIPA | canonical route permissions (`directory.read`, `freeipa.write`, conditional delete) | established identity/security path remains authoritative; `secure-entry.ts` normalizes identity before `freeipa-http-entry.ts`, which owns base reads, query/export/bulk/member/action HTTP behavior | `src/freeipa/**` + route-neutral worker runtimes + private server-side FreeIPA Gateway; FreeIPA owns directory data | `tests/freeipa/**`, `freeipa-http-owner`, permission/route contracts, operation/audit tests for mutations |
...
Public/infrastructure health classifications are intentionally routed before `security-composition.ts`, preserving their historical built-entry availability during schema/security failures. Therefore ordinary production health dispatch does not depend on maintenance state. `worker/maintenance-mode-gate.ts` still preserves its own legacy-health allow/header behavior when exercised directly or through a compatibility path, but that internal gate contract must not be confused with the built-entry health owner.

For non-health traffic, `worker/application.ts` sends classified HTTP requests into `worker/security-composition.ts`. Controlled migration apply/status/reconcile is evaluated first by the explicit storage migration middleware. If that handler returns a response, maintenance is intentionally not consulted, preserving the existing recovery-capable order. All other traffic enters `handleMaintenanceGate`; only traffic that passes maintenance reaches `handleServiceAdminAuthenticationGate`, and only then the remaining compatibility graph starting at `maintenance-control-root-entry.ts`. Non-health negative response finalization happens only after the complete security/compatibility call returns, so the router cannot short-circuit maintenance/service-admin security or replace a maintenance `503` with `404/405`.

Current maintenance behavior is fail-closed when maintenance state cannot be read. Ordinary `/api/**` traffic is rejected while maintenance is active. Maintenance control and verification routes, `/api/schema/status`, storage status/integrity/migration preflight and the public `/api/maintenance/status` compatibility surface retain their documented recovery behavior. Controlled migration apply/status/reconcile remains deliberately outside the maintenance gate and keeps its own guarded authorization/input/error contract. Non-API HTML/static traffic delegates through the maintenance gate. Scheduled work enters `handleMaintenanceScheduledGate` from `security-composition.ts` and is suppressed unless maintenance is inactive, apart from the explicit test-bypass contract.

## Supplemental HTTP surfaces outside `portalRouteContracts`

The stable route registry is intentionally not a universal inventory of every static/infrastructure URL today. The following current surfaces are classified explicitly rather than silently omitted:

| Surface | Current owner | Current role in composition | Application classification / requirement |
| --- | --- | --- | --- |
| `GET /health/dependencies` | `worker/health-http.ts` -> `worker/dependency-health.ts` | sanitized external FreeIPA/XYOps dependency state; pre-schema pass-through | `dependency-health`; application-owned before security |
| `/diagnostics/health`, `/diagnostics/health.js`, `/diagnostics/health.css` | `worker/health-http.ts` -> `worker/health-diagnostics-ui.ts` | schema-independent sanitized incident UI/assets with restrictive CSP | diagnostics supplemental surfaces; application-owned before security |
...
| `/_vinext/image` | `worker/index.ts` | Vinext image optimization adapter | `vinext-image`; Vinext owner remains handler |
| HTML application routes and ordinary static/RSC fallback | `worker/index.ts` -> Vinext handler | UI hosting / route-to-root compatibility behavior | `framework`; preserve until #635 |
...
- **FreeIPA:** canonical query/projection logic under `src/freeipa/**`; HTTP ownership in `worker/freeipa-http-entry.ts`; route-neutral base-read/action normalization, shared RPC, effective-settings, portal-access and operation-run helpers live in dedicated worker runtimes; private `scripts/freeipa-gateway.mjs` remains the upstream trust boundary; browser code never owns credentials/session cookies.
...
3. `application-router.ts` matches stable API metadata through `portal-route-router.ts` and explicitly classifies known-path method mismatches, supplemental surfaces, unknown `/api/**` paths and framework traffic;
4. health stable/supplemental/method-mismatch classifications are offered to `health-http.ts` before security, preserving their historical pre-security availability and handler-specific response contracts;
5. every non-health classification enters `security-composition.ts` without mutating `Request`, environment or execution context;
...
9. `backup-selective-restore-root-entry.ts` owns selective/encrypted/preview backup predispatch and delegates non-backup traffic to `session-management-entry.ts`, keeping backup ownership independent from FreeIPA;
10. after the existing local/settings compatibility path, `secure-entry.ts` performs proxy/static/workspace identity normalization and delegates to `freeipa-http-entry.ts`; that adapter owns base users/groups reads, query/filter projection, CSV export, bulk orchestration, group-member projection, mutation actions and status permission projection, while unrelated integration/status/XYOps/settings traffic continues to `worker/index.ts`;
...
This checkpoint deliberately does **not** move protected non-health 404/405 status decisions ahead of security execution. Phase #631 also deliberately does not hoist FreeIPA handlers toward `application.ts`: it removes wrapper depth and central FreeIPA ownership while preserving the established identity/security position. FreeIPA base reads/actions now reuse dedicated RPC/settings/access/run helpers instead of duplicating those concerns in `worker/index.ts`.
...
- health pre-schema exceptions and application handler ownership consume one canonical classification rather than independent URL lists;
...
- FreeIPA base-read/query/export/bulk/member/action behavior preserves the same protected identity/permission path; bulk mutations reuse the adapter-local action handler instead of recursing into the central runtime;
...
- FreeIPA HTTP ownership is extracted, but shared settings/XYOps compatibility behavior still keeps `worker/index.ts` large and relies on the same portal-access/operation-run runtimes that later #632/#633 slices must continue to reuse;
...
- how later #632/#633 slices should consume the shared portal-access/operation-run/settings helpers without reintroducing local copies while reducing the remaining `worker/index.ts` responsibilities;
...
- `worker/application.ts`
- `worker/application-router.ts`
- `worker/health-http.ts`
- `worker/health-contracts.ts`
...
- `worker/freeipa-base-read.ts`, `worker/freeipa-action-runtime.ts` and `worker/freeipa-rpc.ts`
- `worker/integration-settings-runtime.ts`, `worker/portal-access-runtime.ts` and `worker/operation-run-runtime.ts`
