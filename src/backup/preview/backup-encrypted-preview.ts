...
import type { EncryptedBackupDocument } from "../export/backup-encrypted-export.ts";
import {
  BackupRestorePlanError,
  createBackupRestorePlan,
  type BackupRestorePlan,
} from "../restore/backup-restore-plan.ts";
import {
  BackupRestoreSelectionError,
  selectBackupRestoreDomains,
} from "../restore/backup-restore-selection.ts";
...
export type DecryptedEncryptedBackupSelection = {
  document: ValidatedEncryptedBackupDocument;
  selectedDomains: PortalBackupDomain[];
  fullPayloads: ReadonlyMap<PortalBackupDomain, FullBackupDomainPayload>;
  projected: BackupImportDocument;
};
export type EncryptedBackupPreviewResult = BackupImportPreviewResult & {
  restorePlan: BackupRestorePlan;
};

type EncryptedPreviewDependencies = {
  deriveKey?: typeof deriveBackupKey;
  decrypt?: typeof decryptBackupPayload;
...
export type EncryptedPreviewOptions = {
...
function fail(code: string, status: number, message: string): never {
  throw new BackupEncryptedPreviewError(code, status, message);
}
...
export async function validateEncryptedBackupDocument(value: unknown): Promise<ValidatedEncryptedBackupDocument> {
  ...
  if (Object.keys(value).some((key) => !["manifest", "payloads", "summary"].includes(key))
      || !Object.hasOwn(value, "manifest")
      || !Object.hasOwn(value, "payloads")
      || !Object.hasOwn(value, "summary")) {
    fail("backup_request_invalid", 400, "Encrypted backup document contains invalid fields");
  }
  let manifest;
  try {
  ...
  if (manifest.mode !== "encrypted" || !manifest.encryption) {
    fail("backup_mode_unsupported", 422, "Encrypted backup mode is required");
  }
  if (!arraysEqual(manifest.domains, canonicalDomains(manifest.domains))
      || manifest.entries.length !== manifest.domains.length) {
    fail("backup_request_invalid", 400, "Encrypted backup domains and entries must be canonical");
  }
  if (!plainObject(value.payloads)) {
    fail("backup_request_invalid", 400, "Encrypted backup payloads must be an object");
  }
  ...
  for (let index = 0; index < manifest.domains.length; index += 1) {
    ...
    if (entry.domain !== domain || entry.path !== path) {
      fail("backup_request_invalid", 400, "Encrypted backup entries are not canonical");
    }
    expectedPaths.add(path);
    if (!Object.hasOwn(payloads, path)) {
      fail("backup_payload_missing", 422, "Encrypted backup payload is missing");
    }
    let envelope: EncryptedPayloadEnvelope;
    try {
      envelope = validateEncryptedEnvelope(payloads[path]);
    } catch {
      fail("backup_request_invalid", 400, "Encrypted backup payload is invalid");
    }
    ...
    if (entry.bytes !== envelopeBytes || entry.sha256 !== checksum) {
      fail("backup_decryption_failed", 422, "Backup decryption failed");
    }
  ...
  for (const path of Object.keys(payloads)) {
    if (!expectedPaths.has(path)) {
      fail("backup_payload_unexpected", 422, "Encrypted backup contains an unexpected payload");
    }
  }
  ...
  if (!plainObject(value.summary)
      ...
      || !Number.isSafeInteger(value.summary.entries)
      || !Number.isSafeInteger(value.summary.records)
      || !Number.isSafeInteger(value.summary.bytes)
      || Number(value.summary.entries) !== manifest.entries.length
      || Number(value.summary.records) !== records
      || Number(value.summary.bytes) !== bytes) {
  ...
  return {
    ...
    summary: {
      entries: Number(value.summary.entries),
      records: Number(value.summary.records),
      bytes: Number(value.summary.bytes),
    },
  };
}
...
function normalizedDecryptFailure(error: unknown): BackupEncryptedPreviewError {
  ...
  if (error instanceof BackupRestoreSelectionError) {
    return new BackupEncryptedPreviewError(error.code, error.status, "Encrypted backup preview request is invalid");
  }
  if (error instanceof BackupRestorePlanError) {
    return new BackupEncryptedPreviewError(error.code, error.status, error.message);
  }
  if (error instanceof BackupEncryptionError && error.code === "backup_decryption_failed") {
    return new BackupEncryptedPreviewError("backup_decryption_failed", 422, "Backup decryption failed");
  }
  if (error instanceof FullBackupValidationError) {
    return new BackupEncryptedPreviewError("backup_full_payload_invalid", 422, "Encrypted backup payload is invalid");
  }
  return new BackupEncryptedPreviewError("backup_decryption_failed", 422, "Backup decryption failed");
}

function selectedProjectedDocument(
  document: ValidatedEncryptedBackupDocument,
  selectedDomains: PortalBackupDomain[],
  projectedPayloads: Record<string, unknown>,
): BackupImportDocument {
  const entries = selectedDomains.map((domain) => {
    const entry = document.manifest.entries.find((item) => item.domain === domain);
    if (!entry) fail("backup_request_invalid", 400, "Encrypted backup manifest entry is unavailable");
    return entry;
  });
  return {
    manifest: {
...
export async function decryptEncryptedBackupDomains(
  value: unknown,
  password: unknown,
  selectedDomainsValue: unknown,
  dependencies: EncryptedPreviewDependencies = {},
): Promise<DecryptedEncryptedBackupSelection> {
  ...
  try {
    ...
    const key = await (dependencies.deriveKey ?? deriveBackupKey)(
      password,
      encryption.salt,
      encryption.iterations,
    );
    const fullPayloads = new Map<PortalBackupDomain, FullBackupDomainPayload>();
    const projectedPayloads: Record<string, unknown> = {};
    for (const domain of selectedDomains) {
      const path = `domains/${domain}.json`;
      const plaintext = await (dependencies.decrypt ?? decryptBackupPayload)({
        key,
        context: {
        ...
        envelope: document.payloads[path],
      });
      ...
      if (!entry || entry.records !== actualRecords) {
        fail("backup_full_payload_invalid", 422, "Encrypted backup payload record count is inconsistent");
      }
      if (fullPayload.schemaVersion !== document.manifest.schemaVersion) {
        fail("backup_full_payload_invalid", 422, "Encrypted backup payload schema is inconsistent");
      }
      fullPayloads.set(domain, fullPayload);
    ...
    return {
      ...
      selectedDomains,
      fullPayloads,
      projected: selectedProjectedDocument(document, selectedDomains, projectedPayloads),
    };
  } catch (error) {
...
export async function decryptEncryptedBackupDocument(
  ...
  password: unknown,
  dependencies: EncryptedPreviewDependencies = {},
): Promise<BackupImportDocument> {
  const selected = await decryptEncryptedBackupDomains(value, password, undefined, dependencies);
  return selected.projected;
}

export async function previewEncryptedBackupImport(
  env: BackupExportEnv,
  value: unknown,
  password: unknown,
  ...
  dependencies: EncryptedPreviewDependencies = {},
  options: EncryptedPreviewOptions = {},
): Promise<EncryptedBackupPreviewResult> {
  const document = await validateEncryptedBackupDocument(value);
  ...
  try {
    ...
    const preview = await (dependencies.preview ?? previewBackupImport)(
      env,
      selected.projected,
      schema,
      registry,
    );
    const restorePlan = await (dependencies.createPlan ?? createBackupRestorePlan)(
      env,
      document,
