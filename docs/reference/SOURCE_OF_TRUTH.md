...
| Worker application composition | `worker/application-composition-contract.ts` для build/schema/application/scheduled/framework owner metadata и bounded compatibility exceptions; фактические imports/build config остаются executable evidence | `architecture/ARCHITECTURE.md`, `architecture/PROJECT_STRUCTURE.md`, `architecture/WORKER_COMPOSITION.md` | Не является route/permission registry; #636 должен проверять contract против code и запрещать новые неучтённые compatibility wrappers |
