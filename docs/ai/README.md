...
### Mechanical PR collision gate

`PR Collision Guard / ownership-collision` является дополнительной механической защитой от пропущенной координации. Он сравнивает изменённые файлы PR с другими открытыми PR, направленными в `main`.

- `BLOCKING` означает, что два PR одновременно меняют один exact canonical/high-conflict owner. Такой check нельзя обходить или ослаблять ради merge.
- `INFO` означает обычное exact-file пересечение вне blocking policy. Оно не блокирует CI, но агент должен проверить, действительно ли параллельное редактирование безопасно.
- guard не создаёт новый source of truth и не назначает ownership сам по себе; Issue/PR scope и canonical owners остаются обязательными.

При `BLOCKING` нужно сделать одно из трёх:

1. явно определить dependency/ordering между PR;
2. сузить один scope так, чтобы canonical owner менял только один активный PR;
3. дождаться merge owning PR и replay зависимого изменения на текущий `main`.

Не добавляйте исключение в blocking policy только потому, что две активные ветки уже пересеклись. Сначала устраните конфликт ownership.
...
13. PR collision check не обходится; blocking conflict имеет явный ordering/resolution;
14. PR содержит evidence и known gaps без ложного claim о полном покрытии.
