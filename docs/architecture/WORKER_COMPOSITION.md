...
#633 checkpoint A moves `settings.read` and `settings.test` into the existing `worker/settings-lifecycle-entry.ts` owner, including its internal effective/draft validation calls, while preserving the established local-admin/service-admin security boundary. The legacy direct `PUT /api/integrations/settings`, source/reset compensation flow and revision wrapper remain separate for the next bounded #633 slices; no second settings source of truth is introduced.
