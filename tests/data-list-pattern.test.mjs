...
test("data-list components remain domain agnostic", async () => {
  ...
  for (const forbidden of ["fetch(", "FreeIPA", "XYOps", "/api/", "localStorage", "sessionStorage"]) assert.equal(all.includes(forbidden), false, `domain coupling: ${forbidden}`);
});

test("data table keeps native semantics and an accessible named scroll region", async () => {
...
test("data-list states are explicit and pagination is accessible", async () => {
  ...
  for (const kind of ["loading", "empty", "filtered-empty", "error", "forbidden"]) assert.ok(state.includes(`"${kind}"`), `missing state kind: ${kind}`);
