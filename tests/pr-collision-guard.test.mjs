...
test("high-conflict policy recognizes exact canonical owners", () => {
  for (const path of [
    ...
    "src/auth/local-auth.ts", "src/auth/local-session-management.ts", "src/auth/admin-session-authorization.ts", "docs/reference/SOURCE_OF_TRUTH.md",
...
test("ordinary exact overlap is informational and does not block", () => {
  const result = analyzePullRequestCollisions({ currentPrNumber: 200, currentFiles: ["app/ui/Button.tsx"], pullRequests: [openMainPr(201, "other UI work", ["app/ui/Button.tsx"])] });
  assert.equal(result.blocking, false);
  assert.deepEqual(result.overlaps, [{ severity: "INFO", path: "app/ui/Button.tsx", pullRequests: [{ number: 201, title: "other UI work" }] }]);
});
...
test("canonical overlap blocks and current PR never collides with itself", () => {
  const result = analyzePullRequestCollisions({
    ...
    pullRequests: [openMainPr(200, "current", ["app/page.tsx"]), openMainPr(201, "home extraction", ["app/page.tsx"]), openMainPr(202, "planning", ["docs/superpowers/plans/current.md"])],
  });
  assert.equal(result.blocking, true);
  assert.deepEqual(result.overlaps, [
    { severity: "BLOCKING", path: "app/page.tsx", pullRequests: [{ number: 201, title: "home extraction" }] },
    { severity: "INFO", path: "docs/superpowers/plans/current.md", pullRequests: [{ number: 202, title: "planning" }] },
  ]);
});

test("non-open and non-main pull request entries are ignored", () => {
  const result = analyzePullRequestCollisions({ currentPrNumber: 200, currentFiles: ["worker/index.ts"], pullRequests: [
...
test("multiple conflicts are deduplicated and sorted deterministically", () => {
  const result = analyzePullRequestCollisions({ currentPrNumber: 200, currentFiles: ["package.json", "app/ui/Button.tsx", "db/portal-schema.ts"], pullRequests: [
    openMainPr(205, "dependency work", ["package.json"]), openMainPr(203, "schema work", ["db/portal-schema.ts", "package.json"]),
    openMainPr(204, "button work", ["app/ui/Button.tsx"]), openMainPr(202, "also dependency work", ["package.json", "package.json"]),
  ] });
  assert.deepEqual(result.overlaps, [
    { severity: "BLOCKING", path: "db/portal-schema.ts", pullRequests: [{ number: 203, title: "schema work" }] },
    { severity: "BLOCKING", path: "package.json", pullRequests: [{ number: 202, title: "also dependency work" }, { number: 203, title: "schema work" }, { number: 205, title: "dependency work" }] },
    { severity: "INFO", path: "app/ui/Button.tsx", pullRequests: [{ number: 204, title: "button work" }] },
  ]);
});

test("report formatting distinguishes clean, informational and blocking results", () => {
  assert.equal(formatCollisionReport({ blocking: false, overlaps: [] }), "PR ownership collision check: no overlapping files with other open PRs targeting main.\n");
  assert.match(formatCollisionReport({ blocking: false, overlaps: [{ severity: "INFO", path: "app/ui/Button.tsx", pullRequests: [{ number: 201, title: "button work" }] }] }), /INFO app\/ui\/Button\.tsx -> #201 button work/u);
  const blockingReport = formatCollisionReport({ blocking: true, overlaps: [{ severity: "BLOCKING", path: "worker/index.ts", pullRequests: [{ number: 202, title: "worker refactor" }] }] });
